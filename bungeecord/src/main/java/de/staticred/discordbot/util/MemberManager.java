...
public class MemberManager {
    ...
    public static Member getMemberFromPlayer(UUID uuid) throws SQLException {
        ...
        List<Guild> guilds = DBVerifier.getInstance().jda.getGuilds();

        if (guilds.size() == 0) {
            throw new IllegalStateException("Can't retrieve any guild via. the bot. This must be an error on your side. Be sure the bot is on your discord");
        }

        Guild mainGuild = guilds.get(0);

        User user = mainGuild.getJDA().getUserById(id);
        ...
        if (!mainGuild.isMember(user)) {
            VerifyDAO.INSTANCE.removeDiscordID(uuid);
            return null;
        }

        return mainGuild.getMemberById(id);
    }
